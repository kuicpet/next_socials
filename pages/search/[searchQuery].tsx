  ...
  return (
    <div>
         <div className='flex gap-10 mb-10 mt-10 border-b-2 border-gray-200 bg-white w-full'>
            <p className={`text-xl font-semibold cursor-pointer nt-2 ${accounts}`} onClick={() => setIsAccounts(true)}>Accounts</p>
            <p className={`text-xl font-semibold cursor-pointer nt-2 ${isVideos}`} onClick={() => setIsAccounts(false)}>Videos</p>
        </div>
        {isAccounts ? (
...
export const getServerSideProps = async ({params: {searchQuery}}: {params: {searchQuery: string}}) => {
